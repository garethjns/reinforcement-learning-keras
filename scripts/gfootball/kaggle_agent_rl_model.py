...
This is the pretrained + rl training version of the model. Note that it just loads and uses the keras model, not the
whole DQN agent.
"""
import collections
import os
...
def agent(obs):
    ...
    for b in buffered_obs:
        if b is None:
            ...
            s115_obs.append(Simple115StateWrapper.convert_observation(b['players_raw'], fixed_positions=False))

    obs = np.concatenate([np.array(s)[..., None] for s in s115_obs], axis=2)
