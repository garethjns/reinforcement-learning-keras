...
class TestDeepQAgent(TestRandomAgent):
    ...
    def _agent_specific_set_up(self):
        ...
        self._expected_model_update_after_training_episode: int = 1
    ...
    def _assert_model_unchanged(self, agent: DeepQAgent, checkpoint: List[np.ndarray]):
    ...
    def _assert_model_changed(self, agent: DeepQAgent, checkpoint: List[np.ndarray]):
        action_weights = agent._action_model.get_weights()
        value_weights = agent._value_model.get_weights()
